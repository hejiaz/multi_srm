...
# # different_TR
# in_path = options['output_path']+'accu/different_TR/{}/{}_feat{}_ntrain{}_rand{}_{}_ds{}_{}portion.pickle'
# out_path = options['output_path']+'accu_bar/different_TR/{}/' # {} is roi
# model_all = ['multi_srm']
...
#             accu_all = np.zeros((len(tst_ds),len(rand)),dtype=np.float32)
#             for r,initseed in enumerate(rand):
#                 with open(in_path.format(model,roi,nfeat[m],num_train,initseed,expopt_all[0],ds,port),'rb') as fid:
#                     accu1 = pickle.load(fid)
#                 with open(in_path.format(model,roi,nfeat[m],num_train,initseed,expopt_all[1],ds,port),'rb') as fid:
#                     accu2 = pickle.load(fid)
#                 for d in range(len(tst_ds)):
#                     accu_tmp = (accu1[d]+accu2[d])/2
#                     accu_all[d,r] = np.mean(accu_tmp)
...
#             accu_se = stats.sem(accu_all,axis=1)
#             for d in range(len(tst_ds)):
#                 np.savez_compressed(op+model+'_ds'+str(tst_ds[d])+'_port'+str(port)+'.npz', mean=accu_mean[d],se=accu_se[d])
#     print (op)
