...
roi_all = ['dmn','pt','eac']
model_all = ['multi_srm','all_srm','all_ica','all_gica','all_dict','avg']

# mapping (average across subjects in each dataset)
in_path = options['output_path']+'accu/mapping/{}/{}_feat{}_rand{}_{}_ds{}.npz'
out_path = options['output_path']+'accu_bar/mapping/{}/' # {} is roi
    ...
    for model in model_all:
        ...
        for r,initseed in enumerate(rand):
            for expopt in expopt_all:
                ws = np.load(in_path.format(model,roi,nfeat[m],initseed,expopt,ds))
