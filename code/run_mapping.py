...
with open(options['input_path']+'multi_srm/word_embedding{}_all.pickle'.format(word_dim),'rb') as fid:
...
if model not in ['avg']:
	...
	for d in range(ndata):
		...
		loo.append(loo_tmp)
	# # save W
	# if not os.path.exists(options['output_path']+'W/mapping{}/'+model+'/'.format(word_dim)):
	# 	os.makedirs(options['output_path']+'W/mapping{}/'+model+'/'.format(word_dim))
	# with open(options['output_path']+'W/mapping{}/'+model+'/'+'{}_feat{}_ntrain{}_rand{}_{}_ds{}.pickle'.format(word_dim,roi,nfeature,num_train,initseed,expopt,ds),'wb') as fid:
...
# save results
if not os.path.exists(options['output_path']+'accu/mapping{}/'+model+'/'.format(word_dim)):
	os.makedirs(options['output_path']+'accu/mapping{}/'+model+'/'.format(word_dim))
np.savez_compressed(options['output_path']+'accu/mapping{}/'+model+'/'+'{}_chunks{}_feat{}_ntrain{}_rand{}_{}_ds{}.npz'.format(word_dim,roi,num_chunks,nfeature,num_train,initseed,expopt,ds),accu_class=accu_class,accu_rank=accu_rank)
