...
# shared: number of shared subjects
def run_expt(nfeature,initseed,expopt,model,roi,ds,shared):
	# parameters
	expt = 'mysseg'
	niter = 25

	roi_dict = {'pmc':813,'pt':1318,'eac':1189,'dmn':2329}
	nvoxel = roi_dict[roi]
	print (roi)
	print ('ds1: '+str(ds[0])+', ds2: '+str(ds[1])) # dataset 4 includes 8 datasets
	...
	if ds[0] != 4:
		...
		TR_1st = []
		for d in range(4,12):
			TR_1st.append(int(data_tmp[d].shape[1]/2))
	...
	if ds[0] != 4:
		...
		# use half TR of dataset 1 for alignment, the other half for prediction
		data_1st = np.empty((nvoxel,0,31),dtype=np.float32)
		data_2nd = np.empty((nvoxel,0,31),dtype=np.float32)
		for d in range(4,12):
			data_1st = np.concatenate((data_1st,data_tmp[d][:,:TR_1st[d-4],:]),axis=1)
			data_2nd = np.concatenate((data_2nd,data_tmp[d][:,TR_1st[d-4]:,:]),axis=1)
		if expopt == '1st':			
			data_align.append(ut.zscore_data_all(data_1st))
			data_pred.append(ut.zscore_data_all(data_2nd))
		elif expopt == '2nd':
			data_pred.append(ut.zscore_data_all(data_1st))
			data_align.append(ut.zscore_data_all(data_2nd))	
		else:
			raise Exception('expopt has to be 1st or 2nd')
	...
	if ds[1] != 4:
		...
		data_ds1 = np.empty((nvoxel,0,31),dtype=np.float32)
		for d in range(4,12):
			data_ds1 = np.concatenate((data_ds1,data_tmp[d]),axis=1)
		data_align.append(ut.zscore_data_all(data_ds1))
	...
	# only keep 1 shared subject
	shared_subj = copy.copy(idx2)
	random.shuffle(shared_subj)
	for subj in shared_subj[:-shared]:
		to_be_removed = list(membership[:,1]).index(subj)
		membership[to_be_removed,1] = -1

	idx2 = []
	idx1 = []
	for i in range(shared):
		idx2.append(shared_subj[-(i+1)])
		idx1.append(membership[list(membership[:,1]).index(idx2[i]),0])

	num_subj = membership.shape[0]

	# all active subjects in ds[0] and ds[1]
	subj_ds0 = []
	subj_ds1 = []
	for m in range(num_subj):
		if membership[m,0] != -1:
			subj_ds0.append(membership[m,0])
		if membership[m,1] != -1:
			subj_ds1.append(membership[m,1])

	idx_diff = [n for n in subj_ds1 if n not in idx2] # index of distinct subjects in ds[1]
	...
	W1 = W[:,:,idx_all]
	# transform
	transformed_pred = ut.transform(data_pred[0][:,:,subj_ds0],W1,model)
	# experiment
	...
	out_file = options['output_path']+'accu/dist_subj/'+model+'/'+'{}_feat{}_rand{}_{}_ds{}_shared{}.npz'.format(roi,nfeature,initseed,expopt,ds,shared)
